...
/**
 * VCS_AWeberServiceProvider
 *
 * Provides specific AWeber information or implementing OAuth.
 * @uses VCS_OAuthServiceProvider
 * @package
 * @version $id$
 */
class VCS_AWeberServiceProvider implements VCS_OAuthServiceProvider {
...
class VCS_AWeberAPIBase {
    ...
    protected function readResponse($response, $url) {
        ...
        if (!empty($response['id'])) {
            return new VCS_AWeberEntry($response, $url, $this->adapter);
        } else if (array_key_exists('entries', $response)) {
            return new VCS_AWeberCollection($response, $url, $this->adapter);
        }
        return false;
    }
}
 ...
 * @version $id$
 */
class VCS_AWeberAPI extends VCS_AWeberAPIBase {
    ...
    public static function getDataFromAweberID($string) {
        ...
        $aweber = new VCS_AweberAPI($consumerKey, $consumerSecret);
    ...
    public function setAdapter($adapter=null) {
        if (empty($adapter)) {
            $serviceProvider = new VCS_AWeberServiceProvider();
            $adapter = new VCS_OAuthApplication($serviceProvider);
     ...
     * provided $callbackUrl with the new VCS_token
