...
class VCS_AWeberEntry extends VCS_AWeberResponse {
    ...
    public function attrs() {
        ...
        if (!empty(VCS_AWeberAPI::$_collectionMap[$this->type])) {
            foreach (VCS_AWeberAPI::$_collectionMap[$this->type] as $child) {
     ...
     *
     * @access public
     * @param VCS_AWeberEntry(List)   List to move Resource (this) too.
     * @return mixed VCS_AWeberEntry(Resource) Resource created on List ($list)
    ...
    public function move($list, $last_followup_message_number_sent=NULL) {
        ...
        # Return new VCS_Resource
        $url = $data['Location'];
        $resource_data = $this->adapter->request('GET', $url);
        return new VCS_AWeberEntry($resource_data, $url, $this->adapter);
    }
    ...
    public function __get($value) {
        ...
        if (!empty($this->data) && array_key_exists($value, $this->data)) {
            if (is_array($this->data[$value])) {
                $array = new VCS_AWeberEntryDataArray($this->data[$value], $value, $this);
        ...
        throw new VCS_AWeberResourceNotImplemented($this, $value);
    }
    ...
    public function findSubscribers($search_data) {
        ...
        return new VCS_AWeberCollection($data, $url, $this->adapter);
    }
    ...
    public function getParentEntry(){
        ...
        try {
            ...
            return new VCS_AWeberEntry($data, $url, $this->adapter);
        } catch (Exception $e) {
            return NULL;
        }
    }
    ...
    protected function _parseNamedOperation($data) {
        ...
        foreach($data as $entryData) {
            $results[] = new VCS_AWeberEntry($entryData, str_replace($this->adapter->app->getBaseUri(), '',
    ...
    protected function _methodFor($entryTypes) {
        ...
        throw new VCS_AWeberMethodNotImplemented($this);
    }
    ...
    protected function _getCollection($value) {
        if (empty($this->_collections[$value])) {
            $url = "{$this->url}/{$value}";
            $data = $this->adapter->request('GET', $url);
            $this->_collections[$value] = new VCS_AWeberCollection($data, $url, $this->adapter);
        }
    ...
    protected function _isChildCollection($value) {
        $this->_type();
        if (!empty(VCS_AWeberAPI::$_collectionMap[$this->type]) &&
            in_array($value, VCS_AWeberAPI::$_collectionMap[$this->type])) return true;
        return false;
    }
