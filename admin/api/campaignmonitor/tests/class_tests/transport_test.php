...
@Mock::generate('VCS_CS_REST_SocketWrapper');
@Mock::generatePartial(
    'VCS_CS_REST_SocketTransport', 
    'PartialSocketTransport',
    array('_build_request', '_get_status_code')
);

class VCS_CS_REST_TestSocketTransport extends UnitTestCase {
    var $mock_wrapper;
    var $mock_log;
    ...
    function setUp() {
        $this->mock_log = new MockVCS_CS_REST_Log($this);
        $this->mock_wrapper = new MockVCS_CS_REST_SocketWrapper($this);

        $this->transport = new VCS_CS_REST_SocketTransport($this->mock_log, $this->mock_wrapper);
        $this->partial = new PartialSocketTransport($this);
        $this->partial->VCS_CS_REST_SocketTransport($this->mock_log, $this->mock_wrapper);
    }
