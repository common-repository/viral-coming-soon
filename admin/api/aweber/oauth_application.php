 ...
 * interface are made so that it can interact with our VCS_OAuthApplication
 ...
 * @version $id$
 */
interface VCS_OAuthServiceProvider {
 ...
 * OAuthServiceProvider should have a complementary VCS_OAuthApplication
 *
 * The VCS_OAuthApplication class should contain any details on preparing
 ...
 * as a model.  An VCS_OAuthApplication built to work with a different service
 ...
 * @version $id$
 */
class VCS_OAuthApplication implements VCS_AWeberOAuthAdapter {
    public $debug = false;
    ...
    // Data binding this VCS_OAuthApplication to the consumer application it is acting
    // as a proxy for
     ...
     * __construct
     *
     * Create a new VCS_OAuthApplication, based on an OAuthServiceProvider
     * @access public
     * @return void
     */
    public function __construct($parentApp = false) {
        if ($parentApp) {
            if (!is_a($parentApp, 'VCS_OAuthServiceProvider')) {
                throw new Exception('Parent App must be a valid OAuthServiceProvider!');
            }
            $this->app = $parentApp;
        }
        $this->user = new VCS_OAuthUser();
        $this->curl = new VCS_CurlObject();
    }
    ...
    public function request($method, $uri, $data = array(), $options = array()) {
        ...
        if (empty($options['allow_empty']) && !isset($data)) {
            throw new VCS_AWeberResponseError($uri);
        }
        return $data;
    }
     ...
     * getRequestToken
     *
     * Gets a new VCS_request token / secret for this user.
     * @access public
     * @return void
     */
    ...
    public function getAccessToken() {
        ...
        if (empty($data['oauth_token'])) {
            throw new VCS_AWeberOAuthDataMissing('oauth_token');
        }
    ...
    public function parseAsError($response) {
        if (!empty($response['error'])) {
            throw new VCS_AWeberOAuthException($response['error']['type'],
    ...
    protected function requiredFromResponse($data, $requiredFields) {
        foreach ($requiredFields as $field) {
            if (empty($data[$field])) {
                throw new VCS_AWeberOAuthDataMissing($field);
            }
        }
    }
    ...
    public function makeRequest($method, $url, $data=array()) {
        ...
        if (!$resp) {
            ...
            throw new VCS_AWeberAPIException($error, $url);
        }

        if($resp->headers['Status-Code'] >= 400) {
            $data = json_decode($resp->body, true);
            throw new VCS_AWeberAPIException($data['error'], $url);
        }
    ...
    private function _sendRequest($handle, $headers = array('Expect:')) {
        ...
        if ($resp) {
            return new VCS_CurlResponse($resp);
        }
