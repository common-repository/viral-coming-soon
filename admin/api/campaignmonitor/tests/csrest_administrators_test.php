...
class VCS_CS_REST_ApiKeyTestAdministrator extends VCS_CS_REST_TestAdministrator {
    var $auth = array('api_key' => 'not a real api key');
}

class VCS_CS_REST_OAuthTestAdministrator extends VCS_CS_REST_TestAdministrator {
    var $auth = array(
        'access_token' => '7y872y3872i3eh',
        'refresh_token' => 'kjw8qjd9ow8jo');
}

abstract class VCS_CS_REST_TestAdministrator extends VCS_CS_REST_TestBase {
    var $admins_base_route;

    function set_up_inner() {
        $this->admins_base_route = $this->base_route.'admins';
        $this->wrapper = new VCS_CS_REST_Administrators($this->auth, $this->protocol, $this->log_level,
    ...
    function testupdate() {
        ...
        $expected_result = new VCS_CS_REST_Wrapper_Result($raw_result, 200);
    ...
    function testget() {
        ...
        $expected_result = new VCS_CS_REST_Wrapper_Result($deserialised, $response_code);
