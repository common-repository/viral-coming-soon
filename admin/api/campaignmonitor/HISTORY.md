...
* Added support for new segments structure
  ...
  * The Add Rule call is now Add Rule Group, taking a `ruleGroup` argument instead of a `rule` argument.
    ...
    ```php
    function VCS_CS_REST_Segments->add_rulegroup($rulegroup)
    ```
...
* Added support for authenticating using OAuth. See the [README](README.md#authenticating) for full usage instructions.
  * This introduces some changes to how you authenticate using this library. You now authenticate by passing an `$auth` array as the first argument when creating instances of any classes which inherit from the `VCS_CS_REST_Wrapper_Base` class.

      So in existing code, when you _previously_ would have authenticated using an API key as follows:

      ```php
      $wrap = new VCS_CS_REST_General('Your API Key');
      ...
      ```php
      $wrap = new VCS_CS_REST_General(array('api_key' => 'Your API Key'));
      $result = $wrap->get_clients();
      ```
...
* Added VCS_CS_REST_Campaigns.get_email_client_usage().
...
* Fixed timeout issue by setting VCS_CS_REST_SOCKET_TIMEOUT to 10 seconds.
...
* Removing deprecated method VCS_CS_REST_Clients.set_access().
