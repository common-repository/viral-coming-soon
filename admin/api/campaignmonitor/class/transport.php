...
define('VCS_CS_REST_GET', 'GET');
define('VCS_CS_REST_POST', 'POST');
define('VCS_CS_REST_PUT', 'PUT');
define('VCS_CS_REST_DELETE', 'DELETE');
if (false === defined('VCS_CS_REST_SOCKET_TIMEOUT')) {
    define('VCS_CS_REST_SOCKET_TIMEOUT', 10);
}
if (false === defined('VCS_CS_REST_CALL_TIMEOUT')) {
    define('VCS_CS_REST_CALL_TIMEOUT', 10);
}

if ( ! function_exists('VCS_CS_REST_TRANSPORT_get_available') ) :
function VCS_CS_REST_TRANSPORT_get_available($requires_ssl, $log) {
    if(function_exists('curl_init') && function_exists('curl_exec')) {
        return new VCS_CS_REST_CurlTransport($log);
    } else if(VCS_CS_REST_TRANSPORT_can_use_raw_socket($requires_ssl)) {
        return new VCS_CS_REST_SocketTransport($log);
    } else { 
        $log->log_message('No transport is available', __FUNCTION__, VCS_CS_REST_LOG_ERROR);
...
if ( ! function_exists('VCS_CS_REST_TRANSPORT_can_use_raw_socket') ) :
function VCS_CS_REST_TRANSPORT_can_use_raw_socket($requires_ssl) {
...
if ( ! class_exists('VCS_CS_REST_BaseTransport') ) :
class VCS_CS_REST_BaseTransport {
    
    var $_log;
    
    function VCS_CS_REST_BaseTransport($log) {
        $this->_log = $log;
    }
    ...
    function split_and_inflate($response, $may_be_compressed) {        
        ...
        if($may_be_compressed && preg_match('/^Content-Encoding:\s+gzip\s+$/im', $headers)) {        
            ...
            $this->_log->log_message('Inflated gzipped response: '.$original_length.' bytes ->'.
                strlen($result).' bytes', get_class(), VCS_CS_REST_LOG_VERBOSE);
        }
 ...
 * @since 1.0
 */
if ( ! class_exists('VCS_CS_REST_CurlTransport') ) :
class VCS_CS_REST_CurlTransport extends VCS_CS_REST_BaseTransport {

    var $_curl_zlib;

    function VCS_CS_REST_CurlTransport($log) {
        $this->VCS_CS_REST_BaseTransport($log);
    ...
    function make_call($call_options) {
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, VCS_CS_REST_SOCKET_TIMEOUT);
        curl_setopt($ch, CURLOPT_TIMEOUT, VCS_CS_REST_CALL_TIMEOUT);
        ...
        if(!$response && $response !== '') {
            $this->_log->log_message('Error making request with curl_error: '.curl_errno($ch),
                get_class($this), VCS_CS_REST_LOG_ERROR);
...
if ( ! class_exists('VCS_CS_REST_SocketWrapper') ) :
class VCS_CS_REST_SocketWrapper {
    var $socket;

    function open($domain, $port) {
        $this->socket = fsockopen($domain, $port, $errno, $errstr, VCS_CS_REST_SOCKET_TIMEOUT);

        if(!$this->socket) {
            die('Error making request with '.$errno.': '.$errstr);
            return false;
        } else if(function_exists('stream_set_timeout')) {
            stream_set_timeout($this->socket, VCS_CS_REST_SOCKET_TIMEOUT);
        }
...
if ( ! class_exists('VCS_CS_REST_SocketTransport') ) :
class VCS_CS_REST_SocketTransport extends VCS_CS_REST_BaseTransport {

    var $_socket_wrapper;

    function VCS_CS_REST_SocketTransport($log, $socket_wrapper = NULL) {
        $this->VCS_CS_REST_BaseTransport($log);

        if(is_null($socket_wrapper)) {
            $socket_wrapper = new VCS_CS_REST_SocketWrapper();
        }
