...
class VCS_CS_REST_ApiKeyTestLists extends VCS_CS_REST_TestLists {
    var $auth = array('api_key' => 'not a real api key');
}

class VCS_CS_REST_OAuthTestLists extends VCS_CS_REST_TestLists {
    var $auth = array(
        'access_token' => '7y872y3872i3eh',
        'refresh_token' => 'kjw8qjd9ow8jo');
}

abstract class VCS_CS_REST_TestLists extends VCS_CS_REST_TestBase {
    var $list_id = 'not a real list id';
    var $list_base_route;

    function set_up_inner() {
        $this->list_base_route = $this->base_route.'lists/'.$this->list_id.'/';
        $this->wrapper = new VCS_CS_REST_Lists($this->list_id, $this->auth, $this->protocol, $this->log_level,
