...
if ( ! function_exists('VCS_CS_REST_SERIALISATION_get_available') ) :
function VCS_CS_REST_SERIALISATION_get_available($log) { 
    $log->log_message('Getting serialiser', __FUNCTION__, VCS_CS_REST_LOG_VERBOSE);
    if(function_exists('json_decode') && function_exists('json_encode')) {
        return new VCS_CS_REST_NativeJsonSerialiser($log);
    } else {
        return new VCS_CS_REST_ServicesJsonSerialiser($log);
    }    
}
endif;

if ( ! class_exists('VCS_CS_REST_BaseSerialiser') ) :
class VCS_CS_REST_BaseSerialiser {

    var $_log;
    
    function VCS_CS_REST_BaseSerialiser($log) { 
        $this->_log = $log;
    }
...
if ( ! class_exists('VCS_CS_REST_DoNothingSerialiser') ) :
class VCS_CS_REST_DoNothingSerialiser extends VCS_CS_REST_BaseSerialiser {
    function VCS_CS_REST_DoNothingSerialiser() {}
...
if ( ! class_exists('VCS_CS_REST_NativeJsonSerialiser') ) :
class VCS_CS_REST_NativeJsonSerialiser extends VCS_CS_REST_BaseSerialiser {

    function VCS_CS_REST_NativeJsonSerialiser($log) {
        $this->VCS_CS_REST_BaseSerialiser($log);
    }
...
if ( ! class_exists('VCS_CS_REST_ServicesJsonSerialiser') ) :
class VCS_CS_REST_ServicesJsonSerialiser extends VCS_CS_REST_BaseSerialiser {
    
    var $_serialiser;
    
    function VCS_CS_REST_ServicesJsonSerialiser($log) {
        $this->VCS_CS_REST_BaseSerialiser($log);
