...
class VCS_CS_REST_ApiKeyTestClients extends VCS_CS_REST_TestClients {
    var $auth = array('api_key' => 'not a real api key');
}

class VCS_CS_REST_OAuthTestClients extends VCS_CS_REST_TestClients {
    var $auth = array(
        'access_token' => '7y872y3872i3eh',
        'refresh_token' => 'kjw8qjd9ow8jo');
}

abstract class VCS_CS_REST_TestClients extends VCS_CS_REST_TestBase {
    var $client_id = 'not a real client id';
    var $client_base_route;

    function set_up_inner() {
        $this->client_base_route = $this->base_route.'clients/'.$this->client_id.'/';
        $this->wrapper = new VCS_CS_REST_Clients($this->client_id, $this->auth, $this->protocol, $this->log_level,
