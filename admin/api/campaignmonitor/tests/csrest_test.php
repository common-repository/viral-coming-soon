...
@Mock::generate('VCS_CS_REST_Log');
@Mock::generate('VCS_CS_REST_NativeJsonSerialiser');
@Mock::generate('VCS_CS_REST_CurlTransport');

class VCS_CS_REST_TestBase extends UnitTestCase {
    var $mock_log;
    var $mock_serialiser;
    var $mock_transport;
    ...
    var $log_level = VCS_CS_REST_LOG_NONE;

    var $base_route;

    function setUp() {
        $this->mock_log = new MockVCS_CS_REST_Log();
        $this->mock_serialiser = new MockVCS_CS_REST_NativeJsonSerialiser();
        $this->mock_transport = new MockVCS_CS_REST_CurlTransport();
    ...
    function set_up_inner() {
        $this->wrapper = new VCS_CS_REST_General($this->auth, $this->protocol, $this->log_level,
    ...
    function general_test($wrapper_function, $call_options, $from_transport,
        ...
        $expected_result = new VCS_CS_REST_Wrapper_Result($from_deserialisation, $response_code);
...
class VCS_CS_REST_ApiKeyTestGeneral extends VCS_CS_REST_TestGeneral {
    var $auth = array('api_key' => 'not a real api key');
}

class VCS_CS_REST_OAuthTestGeneral extends VCS_CS_REST_TestGeneral {
    var $auth = array(
    ...
    function test_static_authorize_url_without_state() {
        ...
        $result = VCS_CS_REST_General::authorize_url($client_id, $redirect_uri, $scope);
    ...
    function test_static_authorize_url_with_state() {
        ...
        $result = VCS_CS_REST_General::authorize_url($client_id, $redirect_uri, $scope, $state);
    ...
    function test_refresh_token_error_when_refresh_token_null() {
        ...
        $this->wrapper = new VCS_CS_REST_General($auth, $this->protocol, $this->log_level,
            $this->api_host, $this->mock_log, $this->mock_serialiser, $this->mock_transport);
    ...
    function test_refresh_token_error_when_no_auth() {
        $this->wrapper = new VCS_CS_REST_General(NULL, $this->protocol, $this->log_level,
            $this->api_host, $this->mock_log, $this->mock_serialiser, $this->mock_transport);
        $this->expectError('Error refreshing token. There is no refresh token set on this object.');
        list($new_access_token, $new_expires_in, $new_refresh_token) =
            $this->wrapper->refresh_token();
...
abstract class VCS_CS_REST_TestGeneral extends VCS_CS_REST_TestBase {
    ...
    function testset_primary_contact() {
        ...
        $expected_result = new VCS_CS_REST_Wrapper_Result($raw_result, $response_code);
